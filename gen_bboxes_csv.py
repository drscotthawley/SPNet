...
def get_ellipse_bb(x, y, major, minor, angle_deg,clip=True,width=512,height=384):  # cx, cy, a, b, angle
...
boxinfo = [get_ellipse_bb(row[0],row[1],row[2],row[3],row[4],width=width,height=height) for row in zip(df['cx'],df['cy'],df['a'],df['b'],df['angle'])]
